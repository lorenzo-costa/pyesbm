...
class CovariateClass:
    ...
    def get_nch(self, clustering=None, num_clusters=None):
        if self.nch is None:
            if clustering is None or num_clusters is None:
                raise ValueError("nch not initialised, clustering and num_clusters must be provided to compute nch.")
    ...
    def _arg_validation(self, alpha_c, covariates, num_nodes):
        ...
        if not isinstance(alpha_c, (int, float, np.ndarray)):
            raise TypeError(f"alpha_c must be int, float or np.ndarray. You provided {type(alpha_c)}")
        
        if not isinstance(num_nodes, int):
            raise TypeError(f"num_nodes must be int. You provided {type(num_nodes)}")

        if not isinstance(covariates, list):
            raise TypeError(f"covariates must be a list. You provided {type(covariates)}")
        
        for cov in covariates:
            if not isinstance(cov, tuple) or len(cov) != 2:
                raise ValueError(f"Each covariate must be a tuple of (name, values). You provided {cov}")
            name, values = cov
            if not isinstance(name, str):
                raise TypeError(f"Covariate name must be a string. You provided {type(name)}")
            if not isinstance(values, (list, np.ndarray)):
                raise TypeError(f"Covariate values must be a list or np.ndarray. You provided {type(values)}")
            if len(values) == 0:
