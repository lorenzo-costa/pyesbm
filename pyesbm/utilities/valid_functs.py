...
def validate_models(
    ...
    for i in range(len(model_list)):
        ...
        params = param_list[i]
        model = model_type(
            Y=Y_train, num_users=Y_train.shape[0], num_items=Y_train.shape[1], **params
        )
        ...
        for j in range(len(val_users_unique)):
            ...
            precision = compute_precision(
                val_users_relevant[val_users_unique[j]], ranks_model[j]
            )
        ...
        if precision_list_model == [] or recall_list_model == []:
            raise Exception(
                "No users with relevant items in the validation set."
        ...
        if verbose > 0:
            ...
            print(
                f"Precision @ {k}: {np.round(precision_model, 4)}, Recall @ {k}: {np.round(recall_model, 4)}"
            )
            if true_users is not None:
