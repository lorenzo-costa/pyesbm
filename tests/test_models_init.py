...
class TestInitMethod:
    ...
    def test_user_clustering_length_mismatch(self):
        ...
        with pytest.raises(
            ValueError, match="user clustering length does not match number of users"
        ):
            Baseline(**params)
    ...
    def test_user_clustering_negative_values(self):
        ...
        with pytest.raises(
            ValueError, match="user clustering must be non-negative integers"
        ):
            Baseline(**params)
    ...
    def test_user_clustering_empty_list(self):
        ...
        with pytest.raises(
            Exception, match="user clustering length does not match number of users"
        ):
            Baseline(**params)
    ...
    def test_item_clustering_length_mismatch(self):
        ...
        with pytest.raises(
            ValueError, match="item clustering length does not match number of items"
        ):
            Baseline(**params)
    ...
    def test_item_clustering_negative_values(self):
        ...
        with pytest.raises(
            ValueError, match="item clustering must be non-negative integers"
        ):
            Baseline(**params)
    ...
    def test_dm_valid_parameters(self):
        ...
        params = self.valid_params.copy()
        params.update(
            {"scheme_type": "DM", "bar_h_users": 10, "bar_h_items": 20, "sigma": -0.5}
        )
    ...
    def test_dm_invalid_bar_h_users_type(self):
        ...
        with pytest.raises(
            TypeError, match="maximum number of clusters users must be integer for DM"
        ):
            Baseline(**params)
    ...
    def test_dm_invalid_bar_h_items_type(self):
        ...
        with pytest.raises(
            TypeError, match="maximum number of clusters items must be integer for DM"
        ):
            Baseline(**params)
    ...
    def test_dm_invalid_bar_h_items_negative(self):
        ...
        params = self.valid_params.copy()
        params.update(
            {"scheme_type": "DM", "bar_h_users": 10, "bar_h_items": -10, "sigma": -0.5}
        )
        with pytest.raises(ValueError):
            Baseline(**params)
    ...
    def test_dm_invalid_sigma_positive(self):
        ...
        with pytest.raises(
            ValueError,
            match=f"sigma for DM should be negative. You provided {params['sigma']}",
        ):
            Baseline(**params)
    ...
    def test_dm_invalid_sigma_zero(self):
        ...
        params = self.valid_params.copy()
        params.update(
            {"scheme_type": "DM", "bar_h_users": 10, "bar_h_items": 20, "sigma": 0}
        )
        with pytest.raises(
            ValueError,
            match=f"sigma for DM should be negative. You provided {params['sigma']}",
    ...
    def test_dp_valid_parameters(self):
        ...
        params = self.valid_params.copy()
        params.update({"scheme_type": "DP", "scheme_param": 2.5})
        obj = Baseline(**params)  # Should not raise

    def test_dp_invalid_scheme_param_type(self):
        """Test DP scheme with invalid scheme_param type"""
        params = self.valid_params.copy()
        params.update({"scheme_type": "DP", "scheme_param": "invalid"})
        with pytest.raises(
            TypeError, match="concentration parameter for DP must be float or int"
        ):
            Baseline(**params)
    ...
    def test_dp_invalid_scheme_param_zero(self):
        ...
        params = self.valid_params.copy()
        params.update({"scheme_type": "DP", "scheme_param": 0})
        with pytest.raises(
            ValueError,
            match=rf"concentration parameter for DP should be positive. You provided {params['scheme_param']}",
        ):
            Baseline(**params)
    ...
    def test_dp_invalid_scheme_param_negative(self):
        ...
        params = self.valid_params.copy()
        params.update({"scheme_type": "DP", "scheme_param": -1.5})
        with pytest.raises(
            ValueError,
            match=rf"concentration parameter for DP should be positive. You provided {params['scheme_param']}",
        ):
    ...
    def test_py_valid_parameters(self):
        ...
        params = self.valid_params.copy()
        params.update({"scheme_type": "PY", "sigma": 0.5, "scheme_param": 1.0})
        obj = Baseline(**params)  # Should not raise

    def test_py_valid_parameters_sigma_zero(self, capsys):
        """Test PY scheme with sigma = 0 (prints warning)"""
        params = self.valid_params.copy()
        params.update({"scheme_type": "PY", "sigma": 0, "scheme_param": 1.0})
        obj = Baseline(**params)  # Should not raise
    ...
    def test_py_invalid_sigma_type(self):
        ...
        params = self.valid_params.copy()
        params.update({"scheme_type": "PY", "sigma": "invalid", "scheme_param": 1.0})
    ...
    def test_py_invalid_sigma(self, sigma):
        ...
        params = self.valid_params.copy()
        params.update({"scheme_type": "PY", "sigma": sigma, "scheme_param": 1.0})
        with pytest.raises(
            ValueError,
            match=rf"provide sigma in \[0, 1\) for PY. You provided {params['sigma']}",
        ):
            Baseline(**params)
    ...
    def test_py_invalid_scheme_param_type(self):
        ...
        params = self.valid_params.copy()
        params.update({"scheme_type": "PY", "sigma": 0.5, "scheme_param": "invalid"})
        with pytest.raises(
            TypeError, match="scheme param must be a float or int for PY"
        ):
            Baseline(**params)
    ...
    def test_py_invalid_scheme_param(self, scheme_param):
        ...
        params = self.valid_params.copy()
        params.update({"scheme_type": "PY", "sigma": 0.5, "scheme_param": scheme_param})
        with pytest.raises(
            ValueError,
            match=rf"scheme param should be < -sigma for PY. You provided {params['scheme_param']}",
        ):
            Baseline(**params)
    ...
    def test_gn_valid_parameters(self):
        ...
        params = self.valid_params.copy()
        params.update({"scheme_type": "GN", "gamma": 0.5})
        obj = Baseline(**params)  # Should not raise
    ...
    def test_gn_invalid_gamma_type(self, gamma):
        ...
        params = self.valid_params.copy()
        params.update({"scheme_type": "GN", "gamma": gamma})
        with pytest.raises(
            TypeError, match=rf"gamma should be a float. You provided {type(gamma)}"
        ):
            Baseline(**params)
    ...
    def test_gn_invalid_gamma_values(self, gamma):
        ...
        with pytest.raises(
            ValueError,
            match=rf"gamma for GN should be in \(0, 1\). You provided {params['gamma']}",
        ):
            Baseline(**params)
    ...
    def test_invalid_scheme_type(self, scheme_type):
        ...
        params = self.valid_params.copy()
        params.update({"scheme_type": scheme_type, "scheme_param": 1.0})
        with pytest.raises(
            Exception, match="scheme type must be one of DP, PY, GN, DM"
        ):
            Baseline(**params)
